...
#define DEMO_NAME "my_unsyn_module"
...
static int demodrv_fasync(int fd, struct file *file, int on)
{
	...
	int ret = fasync_helper(fd, file, on, &device->fasync);
	return ret;
}
static int demodrv_release(struct inode *inode, struct file *file)
{
	printk("%s enter\n",__func__);
	demodrv_fasync(-1, file, 0);
	struct my_private_date *data = file->private_data;
	if(data){
		kfree(data);
...
static int __init simple_char_init(void)
{
	...
	for (i = 0; i < MINOR_COUNT ;i++){
		...
		//非常重要，不执行一下操作，将会引起 fasync_helper 报错。
		mydevice[i]->fasync = NULL;
